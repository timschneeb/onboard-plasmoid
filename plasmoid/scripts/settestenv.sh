...
export QML_IMPORT_PATH=$PWD/plasmoid/lib/plasma/applets/libonboardplasmoid.so:$QML_IMPORT_PATH

if ! [[ $QT_PLUGIN_PATH ]]; then
    if [ -f "$PWD"/onboard-plasmoid/plasmoid/lib/plasma/applets/*onboardplasmoid*.so ]; then
        export QT_PLUGIN_PATH=$PWD/onboard-plasmoid/plasmoid/lib
    elif [ -f "$PWD"/plasmoid/lib/plasma/applets/*onboardplasmoid*.so ]; then
